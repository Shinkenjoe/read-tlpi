...
pub fn into_mut_c_char_vec(bytestring: &[u8]) -> Vec<c_char> {
    let mut char_vec: Vec<c_char> = bytestring
        .iter()
        .map(| c | *c as c_char)
        .collect();              
...
mod tests {
    ...
    fn mut_c_string_compares_equal_to_equal_rust_str(){
        ...
        let mut charvec : Vec<c_char> = crate::into_mut_c_char_vec(origin);
    ...
    fn mut_c_string_is_mutable(){
        let origin = b"Geta";
        let mut char_vec : Vec<c_char> = crate::into_mut_c_char_vec(origin);
